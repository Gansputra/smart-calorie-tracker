        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="grid grid-cols-3 gap-3">
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm text-center">
                    ...
                    @if($latestWeight)
                        <p class="text-2xl font-bold text-slate-800">{{ $latestWeight->weight }}<span
                                class="text-sm text-slate-400">kg</span></p>
                ...
                <div class="bg-white rounded-xl p-4 border border-slate-100 shadow-sm text-center">
                    ...
                    @if($weightChange !== null)
                        <p
                            class="text-2xl font-bold {{ $weightChange < 0 ? 'text-green-600' : ($weightChange > 0 ? 'text-red-500' : 'text-slate-600') }}">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100">
                <div class="p-5 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h3 class="font-bold text-slate-800"><i class="fa-solid fa-chart-line mr-1"></i> Grafik Berat Badan
                        </h3>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                ...
                <form method="POST" action="{{ route('weight-log.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="weight" step="0.1" min="20" max="500" value="{{ old('weight') }}"
                            class="form-input @error('weight') border-red-400 @enderror" placeholder="e.g. 70.5" required>
                        @error('weight')
                    ...
                    <div>
                        ...
                        <input type="text" name="notes" value="{{ old('notes') }}" class="form-input"
                            placeholder="e.g. Setelah olahraga">
                    </div>
                    <button type="submit" class="btn-primary w-full"><i class="fa-solid fa-floppy-disk mr-1"></i>
                        Simpan</button>
                </form>
            </div>
                        ...
                        @foreach($weightLogs as $log)
                            <div class="px-5 py-3 border-b border-slate-50 flex items-center justify-between last:border-0">
                                ...
                                <form method="POST" action="{{ route('weight-log.destroy', $log) }}"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg">
