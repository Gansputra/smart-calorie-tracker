        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100">
                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.users.index') }}" class="text-xs text-primary-600 hover:underline">Lihat Semua
                        →</a>
                </div>
                <div class="p-2">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100">
                <div class="px-5 py-4 border-b border-slate-100">
                    <h3 class="font-bold text-slate-800"><i class="fa-solid fa-clock-rotate-left mr-1"></i> Aktivitas
                        Terbaru</h3>
                </div>
                <div class="p-2">
                    @foreach($recentLogs as $log)
                        <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-xs text-slate-400 truncate">{{ $log->user->name ?? 'Unknown' }} ·
                                    {{ $log->created_at->diffForHumans() }}</p>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <p class="text-xs font-semibold text-primary-600">{{ number_format($log->calories, 0) }} kkal
                                </p>
                                @if($log->ai_detected)
                                    <span class="badge-green text-[10px]"><i class="fa-solid fa-wand-magic-sparkles mr-0.5"></i>
                                        AI</span>
                                @endif
                            </div>
                        </div>
